...
from new_model import OverallModel
import logging
from transformers import BertTokenizer, BertModel

# 设置日志记录
logging.basicConfig(filename='new-training_log.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# 训练和验证函数
def train_and_evaluate(p_values, lr_values, weight_decay_values, pretrained_dnn_model, texts, labels, patience=5, batch_size=128, num_epochs=50):
                ...
                for epoch in range(num_epochs):
                    ...
                    if val_loss < best_epoch_val_loss:
                        ...
                        if epochs_no_improve >= patience:
                            # 保存当前最佳模型
                            checkpoint_filename = f"./models/new_sense_best_model_p{p}_lr{lr}_wd{weight_decay}.pth"
                            torch.save(model.state_dict(), checkpoint_filename)
                            logging.info(f"Early stopping. Model checkpoint saved at epoch {epoch+1}")
                            print(f"Early stopping at epoch {epoch+1}. No improvement for {patience} epochs.")
                            break
                 # 如果没有早停，在最后一个 epoch 检查并保存最佳模型
                final_epoch_filename = f"./models/newsense_final_model_p{p}_lr{lr}_wd{weight_decay}_final_epoch.pth"
